...
public class XRayType1 : MonoBehaviour
{
    ...
    void RayCastKeyboard()
    {
...

        {
            ...
            if (OVRInput.GetDown(OVRInput.RawButton.LIndexTrigger) && keyboard.isPressed == false && hit.collider.tag != "Keyboard")
            {
                if (hit.transform.gameObject.layer == LayerBoard)
                {
            ...
            if (OVRInput.GetUp(OVRInput.RawButton.LIndexTrigger) && keyboard.isPressed == true && ColliderHit.transform.parent.localPosition != buttonPosition)
            {
